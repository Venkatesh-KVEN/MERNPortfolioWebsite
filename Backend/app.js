...
app.get('/reset-password/:token', (req, res) => {
  res.send(`
    ...
    <html>
    <head>
      <title>Reset Password :: Kreative Hussain Dashboard</title>
      <style>
        ...
        button { background: #00bc7d; color: white; padding: 12px; border: none; border-radius: 4px; width: 100%; cursor: pointer; font-size: 16px; }
        button:hover { background: oklch(.707 .165 254.624); }
...
const contactLimiter = rateLimit({
  ...
  message: {
    ...
    if (forwardedFor) {
      // The header can be a comma-separated list; the leftmost is the original client
